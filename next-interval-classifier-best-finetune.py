...
model = tflearn.DNN(network, tensorboard_verbose=3, max_checkpoints=5,
                    checkpoint_path='/home/dev/data-science/next-interval-classifier.checkpoints/next-interval-classifier.ft-full.tfl.ckpt')

# Load pre-existing model, restoring all weights, except softmax layer ones
model.load('/home/dev/data-science/next-interval-classifier.best/next-interval-classifier.tfl.ckpt-2250')

# Start finetuning
model.fit(X, Y, n_epoch=10, shuffle=True, validation_set=validationPC,
          show_metric=True, batch_size=200,
          snapshot_epoch=True,
          run_id='next-interval-classifier-1chan-05-finetune-03')

# Save model when training is complete to a file
model.save("next-interval-classifier-1chan-05-finetune-03.tfl")
