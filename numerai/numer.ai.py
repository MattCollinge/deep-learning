...
def build_model():
	...
	# network = conv_1d(network, 128, filter_size=3, strides=1, activation=activation_strat, weights_init=weight_init_strat)

	# network = batch_normalization(network, beta=0.0, gamma=1.0, epsilon=1e-05, decay=0.9, stddev=0.002)
	#
	# network = max_pool_1d(network, 2)
	#fully connected layers
	# network = fully_connected(network, 128, activation=activation_strat, weights_init=weight_init_strat)
	...
	# network = fully_connected(network, 4096, activation=activation_strat, weights_init=weight_init_strat)
	# network = dropout(network, 0.2)
	network = fully_connected(network, 10, activation=activation_strat, weights_init=weight_init_strat)

	network = fully_connected(network, 32, activation=activation_strat, weights_init=weight_init_strat)
	# network = batch_normalization(network, beta=0.0, gamma=1.0, epsilon=1e-05, decay=0.9, stddev=0.002)
	...
	sgd = SGD(learning_rate=0.29, lr_decay=0.999, decay_step=1000, staircase=False)
...
def train(model, data_set_name):
	...
	tflearn.config.init_graph (seed=14636789)
	np.random.seed(seed=14567289)
